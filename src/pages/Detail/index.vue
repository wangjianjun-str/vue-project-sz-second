          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in spuSaleAttrList" :key="item.id">
                ...
                v-for="(attrValue, index) in item.spuSaleAttrValueList" :key="attrValue.id" @click="changeIsCheck(item.spuSaleAttrValueList,index)">{{attrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
              ...
              <div class="add">
                <!-- <router-link to="/addCart" >加入购物车</router-link> -->
...
import { mapGetters, mapState } from 'vuex'
  ...
  export default {
    ...
    mounted(){
      this.getDetailPageInfo()
    },
    methods:{
     getDetailPageInfo(){
       this.$store.dispatch('getDetailPageInfo',this.$route.params.goodsId)
     },
     changeIsCheck(AttrValue,index){
       AttrValue.forEach(item=>{
         item.isChecked  = "0"
       })
       AttrValue[index].isChecked = '1'
     },
    async toSuccess(){
      try {
            let result = await this.$store.dispatch('addorUpdateShopCart',{skuId:this.skuInfo.id, skuNum:this.skuNum})
            alert(result)
            sessionStorage.setItem('SKUINFO_KEY',JSON.stringify(this.skuInfo))
            this.$router.push(`/addcartcussess?skuNum=${this.skuNum}`)
      } catch (error) {
         alert(error.message)
      }
     }
    ...
    computed:{
      ...mapState({
        detailPageInfo: state => state.detail.detailPageInfo
      }),
