<template>
  <div class="cart">
      ...
      <div class="cart-body" >
        <ul class="cart-list" v-for="(shopCart, index) in shopCartList" :key="shopCart.id" >
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" :checked='shopCart.isChecked' @click='updateOneIscheck(shopCart)'>
          </li>
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="text" :value="shopCart.skuNum" minnum="1" class="itxt"  @change="updateCartNum(shopCart,$event.target.value*1)">
          ...
          <li class="cart-list-con7">
            <a href="javascript:;" class="sindelet" @click='deleteCart(shopCart)'>删除</a>
            <br>
    ...
    <div class="cart-tool">
      ...
      <div class="option">
        <a href="#none" @click="deleteAllIsChecked">删除选中的商品</a>
...
import { mapState } from 'vuex'
  export default {
    ...
    async updateCartNum(shopCart,changeNum){
      console.log(shopCart.skuNum)
       if(shopCart.skuNum + changeNum < 1){
          changeNum = 1-shopCart.skuNum
       }
       try {
         let result = await this.$store.dispatch('addorUpdateShopCart',{skuId:shopCart.skuId,skuNum:changeNum})
          this.getShopCartList()
       } catch (error) {
         alert(error.message)
       }
      ...
      //修改单个购物车选中状态信息
      async updateOneIscheck(shopCart){
      try {
      let  result = await this.$store.dispatch('updateIsChecked',{skuId:shopCart.skuId,isChecked:shopCart.isChecked === 1? 0:1})
     ...
     async  deleteCart(shopCart){
       try {
        let result = await this.$store.dispatch('deleteIsChecked',shopCart.skuId)
          alert(result)
          this.getShopCartList()
       } catch (error) {
         alert(error.message)
     ...
     async deleteAllIsChecked(){
       try {
         let result = await this.$store.dispatch('deleteAllIsChecked') 
         alert(result)
         this.getShopCartList()
       } catch (error) {
         alert(error.message)
