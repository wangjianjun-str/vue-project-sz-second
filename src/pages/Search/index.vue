      ...
      <div class="navbar-inner filter">
       <ul class="sui-nav">
            <!-- 
                  ...
                  6、优化代码，主要优化模板当中
                -->
        <li :class='{active:sortFlage === "1"}' @click="changeSort('1')">
         <a href="javascript:;">
           综合
            <i class="iconfont"
            :class='{
            "icon-down":sortType === "desc",
            "icon-up":sortType === "asc"}'
             v-show="sortFlage==='1'"
            ></i>
          </a>
        </li>
        ...
        <li :class='{active:sortFlage === "2"}' @click="changeSort('2')">
         <a href="javascript:;">
           价格
           <i class="iconfont" :class='{
            "icon-down":sortType === "desc",
            "icon-up":sortType === "asc"}' v-show="sortFlage ==='2'"></i>
         </a>
        </li>
       </ul>
      </div>
...
export default {
 ...
 methods: {
  ...
  removeKeyword(){
    ...
    this.searchParams.pageNo = 1
    //使用全局时间总线通知header组件去清空关键字搜索框
    this.$bus.$emit("clearKeyword")
    // this.$router.push({name:"search",query:this.$route.query})
    this.$router.replace({name:"search",query:this.$route.query}) //在search页的跳转都是replace
  //  this.getGoodsListInfo()
  },
  //根据品牌搜索展现数据
  ...
  //
  changeSort(sortFlage){
    // let {order} = this.searchParams
    let originFlage = sortFlage
    let originType = this.searchParams.order.split(":")[1]
    let newOrder
    if(sortFlage === originFlage){
      //点的是同一个按钮
     newOrder = `${sortFlage}:${originType === "desc" ? 'asc' : 'desc'}`

    }else{
      //点的不是同一个
      newOrder = `${sortFlage}:asc`
    }
